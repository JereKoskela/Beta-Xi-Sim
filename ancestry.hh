...
struct Ancestry {
  ...
  double max_fitness(const int island, const int epoch) const {
    ...
    double ret =
        (double)number_of_loci * gsl_matrix_get(selection_rates, epoch, island);
    return ret;
  }

  double fitness(const std::vector<int> &inc, const std::vector<int> &che,
  ...
  int sample_initial_selective_type(const int locus, const int island,
  ...
  int mutate_selective_type(const int child, const int parent,
    ...
    int mut_count = gsl_ran_poisson(
        gen, (branches[parent].leaf_time - branches[child].leaf_time) *
                 selective_mutation_rates[locus]);
  ...
  double sample_neutral_mutation(const int i, double tot_anc_length) const {
    ...
    double ub = (branches[i].ancestral_blocks[block + 1] -
                 branches[i].ancestral_blocks[block]) /
                tot_anc_length;
    while (ub < coin) {
      block += 2;
  ...
  double sample_recombination_point(const int i) const {
    ...
    double ret = branches[i].ancestral_blocks[0] +
                 gsl_rng_uniform(gen) * (branches[i].ancestral_blocks.back() -
                                         branches[i].ancestral_blocks[0]);
    return ret;
  }
  ...
  Ancestry(const char *filename)
    ...
    per_island_locus_recomb_rates =
        gsl_matrix_calloc(number_of_islands, number_of_loci);
  ...
  void read_config(const char *filename) {
    ...
    cfg.lookupValue("effective_population_size", effective_population_size);
    double time_scale = effective_population_size;
    if (alpha < 2.0 && alpha > 1.0) {
      time_scale =
          exp((alpha - 1.0) * (log(alpha) + log(effective_population_size)) -
              alpha * log(alpha - 1.0) - gsl_sf_lnbeta(2.0 - alpha, alpha));
    } else if (alpha <= 1.0) {
    ...
    for (int i = 0; i < number_of_epochs - 1; i++) {
      change_times.push_back((double)root["change_times"][i] / time_scale);
      growth_rates.push_back((double)root["growth_rates"][i] * time_scale);
    }
    growth_rates.push_back((double)root["growth_rates"][number_of_epochs - 1] *
                           time_scale);
    for (int i = 0; i < number_of_loci; i++) {
      ...
      selective_mutation_rates.push_back(
          per_site_mutation_probability *
    ...
    relative_population_sizes =
        gsl_matrix_alloc(number_of_epochs, number_of_islands);
    selection_rates = gsl_matrix_alloc(number_of_epochs, number_of_islands);
    migration_rates = gsl_matrix_alloc(number_of_epochs, number_of_islands);
    population_mergers =
        gsl_matrix_calloc(std::max(number_of_epochs - 1, 1),
                          number_of_islands * number_of_islands);
    int tmp_int = 0;
    for (int i = 0; i < number_of_epochs; i++) {
      for (int j = 0; j < number_of_islands; j++) {
        gsl_matrix_set(
            relative_population_sizes, i, j,
            (double)
                root["relative_population_sizes"][i * number_of_islands + j]);
        gsl_matrix_set(
            selection_rates, i, j,
            (double)root["selection_strengths"][i * number_of_islands + j] *
                time_scale);
        gsl_matrix_set(
            migration_rates, i, j,
            (double)root["migrant_fractions"][i * number_of_islands + j] *
                time_scale);
        if (i < number_of_epochs - 1) {
          for (int k = j + 1; k < number_of_islands; k++) {
            tmp_int = root["population_mergers"]
                          [i * number_of_islands * number_of_islands +
                           j * number_of_islands + k];
            if (tmp_int == 1) {
              gsl_matrix_set(population_mergers, i, j * number_of_islands + k,
                             1);
            }
          }
        }
  ...
  double acceptance_prob(const double impact, const int island) {
    ...
    if (alpha < 2.0) {
      ...
      if (impact > 1e-9) {
        ...
        ret = exp(log(1.0 - exp(ret)) - 2.0 * log(impact) - log(pairs));
      } else {
        int j = 0;
        double tmp = 0.0;
        for (int i = 0; i < number_of_loci; i++) {
          ...
          for (int k = 2; k <= j; k += 2) {
            tmp = (k - 1) * exp(gsl_sf_lnchoose(j, k) + (k - 2) * log(impact));
            if (tmp / ret < 1e-12) {
              break;
            }
            ret += tmp;
  ...
  void simulate_migration_event(const int island, const int target_island,
    ...
    gsl_matrix_set(
        per_island_locus_recomb_rates, island, locus,
        gsl_matrix_get(per_island_locus_recomb_rates, island, locus) -
            recombination_rate(child));
    gsl_matrix_set(
        per_island_locus_recomb_rates, target_island, locus,
  ...
  void simulate_selection_event(const int island, const int locus,
    ...
    for (int j = 0; j < 2; j++) {
      for (int i = 0; i < number_of_loci; i++) {
        ...
        gsl_matrix_set(
            per_island_locus_recomb_rates, island, i,
  ...
  void simulate_recombination_event(const int island, const int locus,
    ...
    int child_ind = 0;
    double upper_bound =
        recombination_rate(active_branches[island][locus][child_ind]);
    double coin = gsl_rng_uniform(gen) *
                  gsl_matrix_get(per_island_locus_recomb_rates, island, locus);
    ...
    break_points[locus].insert(std::upper_bound(break_points[locus].begin(),
                                                break_points[locus].end(),
                                                split_point),
                               split_point);
    int index = 0;
    ...
    points_1.insert(points_1.begin(), branches[child].ancestral_blocks.begin(),
    ...
    if (gsl_rng_uniform(gen) < acceptance_prob(impact, island)) {
      ...
      for (int l = 0; l < number_of_loci; l++) {
        if (l == locus) {
          do {
            size_at_locus =
                2 + gsl_ran_binomial(gen, impact,
                                     active_branches[island][l].size() - 2);
          } while (gsl_rng_uniform(gen) > 1 / gsl_sf_choose(size_at_locus, 2));
        } else {
          size_at_locus =
              gsl_ran_binomial(gen, impact, active_branches[island][l].size());
        }
        if (size_at_locus > 1) {
          int mergers, running_total = 0;
          int actual_mergers = 0;
          for (int i = 0; i < max_families; i++) {
            mergers = gsl_ran_binomial(gen, 1.0 / (double)(max_families - i),
                                       size_at_locus - running_total);
            ...
            if (mergers > 1) {
              ...
              for (int j = 0; j < (int)children.size(); j++) {
                tmp.children.push_back(active_branches[island][l][children[j]]);
                gsl_matrix_set(
                    per_island_locus_recomb_rates, island, l,
                    gsl_matrix_get(per_island_locus_recomb_rates, island, l) -
                        recombination_rate(tmp.children.back()));
                if (branches[tmp.children[j]].virtual_flag == 0) {
                  tmp.virtual_flag = 0;
                }
              }
              std::vector<double> tmp_blocks =
                  branches[tmp.children[0]].ancestral_blocks;
              int ind;
              for (int j = 1; j < mergers; j++) {
                ind = 0;
                for (int k = 0;
                     k <=
                     (int)branches[tmp.children[j]].ancestral_blocks.size() / 2;
                     k += 2) {
                  if (ind < (int)tmp_blocks.size()) {
                    while (tmp_blocks[ind] <
                           branches[tmp.children[j]].ancestral_blocks[k]) {
                      ind += 2;
                      if (ind == (int)tmp_blocks.size()) {
                        break;
                      }
                  ...
                  tmp_blocks.insert(
                      tmp_blocks.begin() + ind,
                      branches[tmp.children[j]].ancestral_blocks.begin() + k,
                      branches[tmp.children[j]].ancestral_blocks.begin() + k +
                          2);
                }
              }
              ...
              while (ind < (int)tmp_blocks.size() - 2) {
                while (tmp_blocks[ind + 2] < tmp_blocks[ind + 1]) {
                  tmp_blocks[ind + 1] =
                      fmax(tmp_blocks[ind + 1], tmp_blocks[ind + 3]);
              ...
              for (int j = mergers - 1; j > -1; j--) {
                branches[tmp.children[j]].parents.push_back(branches.size());
                active_branches[island][l].erase(
                    active_branches[island][l].begin() + children[j]);
              }
              branches.push_back(tmp);
              gsl_matrix_set(
                  per_island_locus_recomb_rates, island, l,
          ...
          for (int i = actual_mergers; i > 0; i--) {
            active_branches[island][l].push_back(branches.size() - i);
          }
        }
      }
    }
  ...
  double time_increment(int &event_type, int &island, int &target_island,
    ...
    for (int i = 0; i < number_of_islands; i++) {
      helpful_factor +=
          pow(gsl_matrix_get(relative_population_sizes, 0, i), 3.0 - alpha);
    }
    for (int i = 0; i < number_of_islands; i++) {
      for (int j = 0; j < number_of_loci; j++) {
        if (active_branches[i][j].size() > 1) {
          if (growth_rates[epoch] * (alpha - 1.0) > 0.0) {
            ...
            tmp *= pow(gsl_matrix_get(relative_population_sizes, epoch, i),
                       alpha - 1.0);
            tmp *= exp(-growth_rates[epoch] * (alpha - 1.0) * sim_time);
            tmp *= helpful_factor * log(gsl_rng_uniform_pos(gen));
            tmp /= (double)(active_branches[i][j].size() *
                            (active_branches[i][j].size() - 1));
            tmp = log(1.0 - tmp) / (growth_rates[epoch] * (alpha - 1.0));
          } else {
        ...
        if (active_branches[i][j].size() > 0) {
          for (int k = 0; k < number_of_islands; k++) {
            if (k != i && gsl_matrix_get(migration_rates, epoch, k) *
                                  migration_probability(k, i, epoch) >
                              0.0) {
              tmp = gsl_matrix_get(relative_population_sizes, epoch, i);
              tmp /= gsl_matrix_get(relative_population_sizes, epoch, k);
              tmp /= (double)active_branches[i][j].size();
          ...
          if (gsl_matrix_get(per_island_locus_recomb_rates, i, j) > 0.0) {
            tmp = gsl_ran_exponential(
                gen, 1.0 / gsl_matrix_get(per_island_locus_recomb_rates, i, j));
            if (tmp < ret) {
              ret = tmp;
              island = i;
              event_type = 3;
  ...
  void simulate_event(int &total_active_branches) {
    ...
    switch (event_type) {
    case 0:
      simulate_coalescence_event(island, locus, total_active_branches);
      break;
    case 1:
      simulate_migration_event(island, target_island, locus);
      break;
    case 2:
      simulate_selection_event(island, locus, total_active_branches);
      break;
    case 3:
      simulate_recombination_event(island, locus, total_active_branches);
      break;
    case 4:
      epoch++;
      for (int i = 0; i < number_of_islands; i++) {
      ...
      break;
    default:
      std::cout << "unrecognised event type" << std::endl;
      abort();
    }
    return;
  }
  ...
  void extract_trees() {
    ...
    for (int i = (int)branches.size() - 1; i > -1; i--) {
      if (branches[i].parents.size() == 0) {
        branches[i].selective_type = sample_initial_selective_type(
            branches[i].locus, branches[i].island, branches[i].leaf_epoch);
      } else {
        if ((int)branches[i].incoming.size() == 2 * number_of_loci) {
          double selection_prob =
              fitness(branches[i].incoming, branches[i].checking,
                      branches[i].island, branches[i].leaf_epoch);
          selection_prob /=
              max_fitness(branches[i].island, branches[i].leaf_epoch);
          if (gsl_rng_uniform(gen) < selection_prob) {
            if (branches[i].virtual_flag == 1) {
              branches[branches[i].parents[0]].children.clear();
              branches[i].parents[0] = branches[i].incoming[branches[i].locus];
              branches[branches[i].parents[0]].children.push_back(i);
            } else {
              branches[branches[i].parents[0]].children.clear();
              ...
              while (to_check.size() > 0) {
                ...
                for (unsigned int j = 0;
                     j < branches[to_check[0]].children.size(); j++) {
                  if (branches[branches[to_check[0]].children[j]]
                          .virtual_flag == 0) {
                    virtual_q = 0;
                    break;
                  }
              ...
              to_check = branches[branches[i].parents[0]].parents;
              while (to_check.size() > 0) {
        ...
        branches[i].selective_type =
            mutate_selective_type(i, branches[i].parents[0], branches[i].locus);
      }
    }
    return;
  }
  ...
  void fill_orders() {
    ...
    int child = 0;
    double mid = 0.0;
    for (unsigned int i = total_sample_size * number_of_loci;
         i < branches.size(); i++) {
      loc = branches[i].locus;
  ...
  void print_normalised_sfs_by_island() const {
    ...
    std::vector<std::vector<std::vector<int>>> sfs_by_island(number_of_islands,
                                                             sfs);
    int loc = 0;
      ...
      if (branches[i].virtual_flag == 0 && branches[i].parents.size() > 0) {
        ...
        for (int j = 0; j < (int)break_points[loc].size() - 1; j++) {
          ...
          if (branches[i].contains(mid) == 1) {
            ...
            for (int k = 0; k < number_of_islands; k++) {
              if (branches[i].order[j][k] > 0 &&
                  branches[i].order[j][k] < sample_size[k]) {
